...
describe("<Blog />", () => {
  test("renders only title & author by default", () => {
    ...
    const { container } = render(
      <Blog blog={blog} likeBlog={() => false} removable={false} />,
    );
    ...
    expect(div).toHaveTextContent("First blog");
    expect(div).toHaveTextContent("Maximilian Hewitt");
  });
  ...
  test("clicking the view button displays the toggledOn div", async () => {
    ...
    const { container } = render(
      <Blog blog={blog} likeBlog={mockHandler} removable={false} />,
    );
